...
JWT_EXPIRES_IN = os.getenv("JWT_EXPIRES_IN", "7d")  # 7 days

# Конвертируем "7d" в минуты для timedelta
def parse_expires_in(expires_in: str) -> int:
    ...
    if expires_in.endswith('d'):
        ...
        return 7 * 24 * 60  # 7 дней по умолчанию

class TokenPayload:
    """Модель payload токена (аналог интерфейса TokenPayload)"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            "sub": str(self.user_id),  # sub = subject (стандартное поле JWT)
            "email": self.email,
            "role": self.role
        }
...
def generate_token(payload: TokenPayload) -> str:
    """Генерация JWT токена (аналог generateToken)"""
...
def verify_token(token: str) -> Optional[TokenPayload]:
    """Проверка JWT токена (аналог verifyToken)"""
    try:
